#!/usr/bin/perl
#
# $Id: tcp-scan-multi.pl 10 2011-11-16 22:03:53Z gomor $
#
use strict;
use warnings;
...
use Getopt::Std;
my %opts;
getopts('t:p:r:n:DF6', \%opts);

if (!$opts{t}) {
   die("Usage: $0 -t targets -p ports -r rate -n ntimes [-D] [-F] [-6]\n");
}
...
if (! $opts{6}) {
   ...
   my $oNet = Net::Netmask->new2($target);
   if (!defined($oNet)) {
      printf("ERROR: Net::Netmask [$!]\n");
      exit(-1);
   }
...
my $dev;
if ($opts{6}) {
   $dev = Net::Frame::Device->new(target6 => $targets[0]);
}
else {
   $dev = Net::Frame::Device->new(target => $targets[0]);
}
...
my %ports = map { $_ => 1 } @top100;
if ($opts{D}) {
   %ports = map { $_ => 1 } @top1000;
}
elsif ($opts{F}) {
   %ports = map { $_ => 1 } @full;
}
...
if ($opts{6}) {
   $d->filter(
      'ip6 and src host '.$target.' and dst host '.$dev->ip6,
   );
}
else {
   $d->filter(
      '((tcp[13] & 2 != 0) and (tcp[13] & 16 != 0) and src net '.$target.
      ') or '.
      '((tcp[13] & 4 != 0) and (tcp[13] & 16 != 0) and src net '.$target.')'
   );
}
$d->start;
...
use Time::Interval;
my $estim  = parseInterval(seconds => $pass * scalar(@send) * scalar(@targets) / $pps);
my $nhosts = scalar(@targets);
printf("Estimated running time: %d day(s) %d hour(s) %d minute(s) %d second(s) for %d host(s)\n",
       $estim->{days},
       $estim->{hours},
       $estim->{minutes},
       $estim->{seconds},
...
while (! $d->timeout) {
   if (my $f = $d->next) {
      ...
      if ($s->ref->{TCP}) {
         my $ip  = $opts{6} ? $s->ref->{IPv6} : $s->ref->{IPv4};
         my $tcp = $s->ref->{TCP};
         if ($tcp->flags == 0x12) { # SYN+ACK
            print STDERR "open: [".$ip->src."]:".$tcp->src."/tcp\n";
            $open{$ip->src}{$tcp->src} = {
               id    => $opts{6} ? $ip->flowLabel : $ip->id,
               ttl   => $opts{6} ? $ip->hopLimit  : $ip->ttl,
               win   => $tcp->win,
               opt   => unpack('H*', $tcp->options),
               flags => 'SA',
            };
            delete $ports{$tcp->src};
         }
